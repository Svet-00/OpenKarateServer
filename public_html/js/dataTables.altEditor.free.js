...
;(function (factory) {
  ...
  $.extend(altEditor.prototype, {
        ...
        if (columnDefs[j].name != null) {
          ...
          if (columnDefs[j].type.indexOf('checkbox') >= 0) {
            if (this._quoteattr(selectedValue) === 'true') {
              $(selector)
                .find(jquerySelector)
                .prop('checked', this._quoteattr(selectedValue)) // required by checkbox
            }
          }
          //added date
          if (columnDefs[j].type.indexOf('date') >= 0) {
            if (columnDefs[j].dateFormat !== '') {
              ...
              if (mDate && mDate.isValid()) {
                $(selector)
                  .find(jquerySelector)
                  .val(mDate.format(columnDefs[j].dateFormat))
              }
            }
          }
        }
    ...
    _editRowData: function () {
      ...
      $('form[name="altEditor-edit-form-' + this.random_id + '"] *')
        ...
        .each(function (i) {
          const shit = this
          if ($(shit).prop('files')[0]) {
            rowDataArray[$(shit).attr('id')] = $(shit).prop('files')[0]
          }
        })
        ...
        if (columnDefs[j].type.indexOf('hidden') >= 0) {
          ...
          if (columnDefs[j].type.indexOf('select') >= 0) {
            ...
            if (fvalue instanceof Array) {
              // multiselect
              var mapped = fvalue.map(mapper).filter(function (x) {
                return x != null
              })
              fvalue = mapped.join(', ')
            } else {
      ...
      for (var j in columnDefs) {
        ...
        if (columnDefs[j].type.indexOf('hidden') >= 0) {
          ...
          // Adding checkbox
            ...
            data += '<div class="custom-control custom-checkbox large">'
            data +=
              "<input class='custom-control-input' type='" +
          ...
          // Adding file input
            ...
            data +=
              "<input class='custom-file-input' type='" +
              this._quoteattr(columnDefs[j].type) +
              "' id='" +
              this._quoteattr(columnDefs[j].name) +
              "' pattern='" +
              ...
              (columnDefs[j].maxLength == false
                ...
                : " maxlength='" + columnDefs[j].maxLength + "'") +
              " accept='" +
              this._quoteattr(columnDefs[j].accept) +
              "'" +
              " style='overflow:hidden;" +
              this._quoteattr(columnDefs[j].style) +
              "'>"
          ...
          // Adding text-inputs and errorlabels, but also new HTML5 types (email, color, ...)
          else {
            data +=
    ...
    _addRowData: function () {
      ...
      $('form[name="altEditor-add-form-' + this.random_id + '"] *')
        ...
        .each(function (i) {
          const shit = this
          console.log(shit)
          if ($(shit).prop('files').length > 0) {
            rowDataArray[$(shit).attr('id')] = $(shit).prop('files')
          }
        })
  ...
  const customFileInputHandler = function (e) {
    ...
    nextSibling.innerText = fileNamesString
  }

  $('.custom-file-input').on('change', customFileInputHandler)

  const config = { attributes: false, childList: true, subtree: true }

  const globalObserver = new MutationObserver(function (
    mutationsList,
    globalObserver
  ) {
    for (const mutation of mutationsList) {
      if (mutation.type === 'childList') {
        const altEditorModals = Array.from(
          document.getElementsByClassName('altEditor-modal')
        )

        if (altEditorModals.length == 2) {
          globalObserver.disconnect()

          altEditorModals.forEach((node) => {
